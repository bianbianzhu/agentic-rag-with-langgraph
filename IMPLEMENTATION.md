...
The next implementation work begins Chapter 09 only: Authorization changes mid-conversation. Read its three linked decisions before planning, then compose the top-level Turn around one current Authorization Snapshot, discard all Current Turn Work on a detected change, restart exactly once from trusted context, and terminate safely on a second change or Principal mismatch. Chapters 01 through 08 are verified; preserve their public seams unless Chapter 09 evidence requires a compatible extension.
