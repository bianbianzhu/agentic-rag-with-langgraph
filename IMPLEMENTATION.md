...
The next implementation session begins Chapter 03 only: Authorization before relevance. Read its two linked decisions before planning, then add trusted Principals, Group-derived Access Grants, default deny, Authorization Snapshots, and the shared SQL authorization predicate with the fixed Alice/Bob/Carol grant matrix. Chapters 01 and 02 are verified; do not alter their public seams unless Chapter 03 evidence requires a compatible extension.
