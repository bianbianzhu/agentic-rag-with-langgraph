...
The next implementation session begins Chapter 04 only: Authorized hybrid retrieval. Read its three linked decisions before planning, then add dense and lexical retrieval that apply the same PostgreSQL authorization predicate before rows leave the database. Chapters 01 through 03 are verified; do not alter their public seams unless Chapter 04 evidence requires a compatible extension.
